 ...
 * @author Raofin
 */
public class UserRegisterJFrame extends javax.swing.JFrame {

    /**
     * Creates new form UserRegisterJFrame
     */
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        try {
            ...
            if (rowsInserted > 0) {
                ...
                // Close the generated keys result set
                generatedKeys.close();

                if (newUserId != -1) {
                    // Insert a new row in the Flats table with the same owner and resident user
                    String insertFlatQuery = "INSERT INTO Flats (OwnerUserId, ResidentUserId) VALUES (?, ?)";
                    PreparedStatement insertFlatStatement = connection.prepareStatement(insertFlatQuery);
                    insertFlatStatement.setInt(1, newUserId);
                    insertFlatStatement.setInt(2, newUserId);

                    int rowsInsertedInFlats = insertFlatStatement.executeUpdate();

                    if (rowsInsertedInFlats > 0) {
                        // Flats table insertion successful
                        JOptionPane.showMessageDialog(null, "Registration and Flat creation successful.", "Success", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        // Flats table insertion failed
                        JOptionPane.showMessageDialog(null, "Registration successful, but failed to create the corresponding Flat.", "Partial Success", JOptionPane.WARNING_MESSAGE);
                    }

                    insertFlatStatement.close();
                } else {
                    // Failed to retrieve the newly inserted user's ID
                    JOptionPane.showMessageDialog(null, "Registration successful, but failed to retrieve the user's ID.", "Partial Success", JOptionPane.WARNING_MESSAGE);
                }
            } else {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(UserRegisterJFrame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
